<?php
// !!! NOTE:
//this file assumes it is being included in another php
...
function generateStaticFiles($forDrive){
	global $driveId, $jsonOutput,$csvOutput,$kmlOutput, $driveDesc, $driveName;
	define('FILE_PATH', realpath(dirname(__FILE__) . '/../'));
	//include db
	include FILE_PATH.'/inc/pdoDB.php';
	$driveId = $forDrive;
	$done = false;
	$iterated = 0;
	$index = 0;
	...
	if($jsonCount > 0 ) {
		...
		//echo FILE_PATH.'/feeds/driveCache/drive'.$driveId.'.json';
		
		//write KML
		try{
			$file = fopen(FILE_PATH .'/feeds/driveCache/drive'.$driveId.'.kml',"w");
			fwrite($file, $kmlOutput); 
			fclose($file); 
		}catch (Exception $e) {
		//	return false;			
		}
		//really should look at zipping KML into KMZ
		
		//write CSV
		try{
			$file = fopen(FILE_PATH .'/feeds/driveCache/drive'.$driveId.'.csv',"w");
			fwrite($file, $csvOutput); 
			fclose($file); 
		}catch (Exception $e) {
			return false;			
		}
		...
		//write JSON
		try{
			$file = fopen(FILE_PATH .'/feeds/driveCache/drive'.$driveId.'.json',"w");
